...
interface DashboardStats {
    ...
    recent_workspaces: { id: string; name: string; created_at: string }[];
}
...
export default function DashboardView() {
    ...
    useEffect(() => {
        fetchApi('/workspaces/stats')
            .then(setStats)
            .catch(() => { }); // non-critical
    }, [workspaces]); // re-fetch when workspaces change
    ...
    const statCards = [
        {
            ...
            value: stats?.workspace_count ?? workspaces.length,
            icon: FolderOpen,
        ...
        <div className="max-w-6xl mx-auto space-y-8 p-6 md:p-8">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1 text-sm md:text-base">
                    Manage your research libraries and track your progress.
                </p>
            </div>
                ...
                {statCards.map((card, i) => (
                    ...
                    >
                        {card.link ? (
                            <Link
                                to={card.link}
                                className="flex flex-col gap-3 p-5 bg-card border border-border/60 hover:border-primary/40 rounded-2xl hover:shadow-lg hover:shadow-primary/5 transition-all group"
                            >
                                ...
                                <div>
                                    ...
                                    <p className={`text-base font-bold ${card.color} group-hover:underline`}>{card.value}</p>
                                </div>
                            </Link>
                        ) : (
                            <div className="flex flex-col gap-3 p-5 bg-card border border-border/60 rounded-2xl">
                                <div className={`w-9 h-9 rounded-lg ${card.bg} flex items-center justify-center`}>
                                    <card.icon className={`w-4 h-4 ${card.color}`} />
                                </div>
                                <div>
            ...
            {/* ── Workspaces Section ───────────────────────────── */}
            <div>
                <h2 className="text-xl font-bold tracking-tight mb-4">Workspaces</h2>
